...
!
! New file:      D:\dev-IES\ExtractProceduresTest\testoutput\Reports.CustInvoiceReport.clw
!
...
!================================================================================
CustInvoiceReport PROCEDURE                                ! Generated from procedure template - Report
...
ORD:OrderNumberBreak     BREAK(ORD:OrderNumber)
                           HEADER,AT(0,0,,1067)
                             STRING(@s20),AT(108,25),USE(CUS:Company)
                             STRING('Order Number:'),AT(3604,42),USE(?String17),TRN
                             STRING(@n-7),AT(4531,52),USE(ORD:OrderNumber),RIGHT(1)
                             STRING(@s20),AT(1717,217),USE(CUS:LastName)
                             STRING(@s20),AT(108,217),USE(CUS:FirstName)
                             STRING(@d1),AT(4533,275),USE(ORD:OrderDate),RIGHT(1)
                             STRING('Order Date:'),AT(3600,233),USE(?String18),TRN
                             STRING(@P#####P),AT(2067,600),USE(CUS:ZipCode),RIGHT(1)
                             ...
                             STRING(@s20),AT(108,408),USE(CUS:Address)
                             STRING(@s20),AT(108,600),USE(CUS:City)
                             STRING(@s2),AT(1758,600),USE(CUS:State)
                           END
detail                     DETAIL
                             STRING(@n-7),AT(75,42),USE(DTL:Quantity),RIGHT(1)
                             STRING(@n-7),AT(771,42),USE(DTL:ProdNumber),RIGHT(1)
                             STRING(@s25),AT(1438,42),USE(PRD:ProdDesc)
                             STRING(@n-7.2),AT(3475,42),USE(DTL:ProdAmount),DECIMAL(12)
                             STRING(@n-10.2),AT(4333,42),USE(ItemTotal),DECIMAL(12)
                           END
                           FOOTER,AT(0,0),PAGEAFTER(-1)
                             LINE,AT(3617,67,1469,-10),USE(?Line2),COLOR(COLOR:Black),LINEWIDTH(20)
                             STRING(@n9.2),AT(4450,100),USE(ItemTotal,,?ItemTotal:2),SUM,RESET(ORD:OrderNumberBreak),TRN
                             STRING('Order Total:'),AT(3575,100),USE(?String23),TRN
                           END
                         END
                         FOOTER,AT(0,0),PAGEAFTER(-1)
                           STRING('Invoice Summary for:'),AT(1475,58),USE(?String25),TRN
                           STRING(@s20),AT(2842,58),USE(CUS:Company,,?CUS:Company:2)
                           STRING(@n-10.2),AT(2900,250),USE(ItemTotal,,?ItemTotal:3),SUM,RESET(CUS:CustNumberBreak),TRN
                           STRING(@n3),AT(2450,250),USE(InvoiceCount),CNT,TALLY(ORD:OrderNumberBreak),RESET(CUS:CustNumberBreak), |
  TRN
                           STRING('Total Orders:'),AT(1533,250),USE(?String26),TRN
                         END
                       END
...
ReturnValue          BYTE,AUTO
  ...
  CODE
  GlobalErrors.SetProcedureName('CustInvoiceReport')
  ...
  INIMgr.Fetch('CustInvoiceReport',ProgressWindow)         ! Restore window settings from non-volatile store
  ...
  ThisReport.AddRange(CUS:CustNumber)
...
ReturnValue          BYTE,AUTO
  ...
  END
  IF SELF.Opened
    INIMgr.Update('CustInvoiceReport',ProgressWindow)      ! Save window data to non-volatile store
  END
  ProgressMgr.Kill()
