...
include('CIDC_Sales_Invoice.inc'),once
   ...
   MAP
     MODULE('CIDC_SALES_UI_INVOICE_TESTS_BC.CLW')
DctInit     PROCEDURE                                      ! Initializes the dictionary definition module
DctKill     PROCEDURE                                      ! Kills the dictionary definition module
     END
!--- Application Global and Exported Procedure Definitions --------------------------------------------
     MODULE('CIDC_SALES_UI_INVOICE_TESTS001.CLW')
NoName                 FUNCTION(*long addr),long,pascal   !
     END
...
CIDC_Sales_UI_Invoice_Tests:Init PROCEDURE(<ErrorClass curGlobalErrors>, <INIClass curINIMgr>)
CIDC_Sales_UI_Invoice_Tests:Kill PROCEDURE
   END
...
DLLInitializer.Construct PROCEDURE
  ...
  LocalINIMgr.Init('.\CIDC_Sales_UI_Invoice_Tests.INI', NVD_INI) ! Initialize the local INI manager to use windows INI file
  INIMgr &= LocalINIMgr
...
ClarionTest_GetListOfTestProcedures PROCEDURE(*LONG Addr)
    ...
    ClarionTest_ctpl.List.TestName       = 'NoName'
...
CIDC_Sales_UI_Invoice_Tests:Init PROCEDURE(<ErrorClass curGlobalErrors>, <INIClass curINIMgr>)
CIDC_Sales_UI_Invoice_Tests:Init_Called    BYTE,STATIC

  CODE
  IF CIDC_Sales_UI_Invoice_Tests:Init_Called
     RETURN
  ELSE
     CIDC_Sales_UI_Invoice_Tests:Init_Called = True
  END
...
CIDC_Sales_UI_Invoice_Tests:Kill PROCEDURE
CIDC_Sales_UI_Invoice_Tests:Kill_Called    BYTE,STATIC

  CODE
  IF CIDC_Sales_UI_Invoice_Tests:Kill_Called
     RETURN
  ELSE
     CIDC_Sales_UI_Invoice_Tests:Kill_Called = True
  END
